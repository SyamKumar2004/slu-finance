...
export default function ProtectedAdminDashboard() {
  ...
  const [adminProfile, setAdminProfile] = useState({ name: 'Potnuru Syamkumar', email: 'syamkumarpotnuru7@gmail.com', role: 'Principal Credit Manager' });
  ...
  async function fetchDynamicRealtimeMetrics() {
    ...
    if (rawLoans) {
      loanList = rawLoans.map((l: any) => ({
        ...
        status: l.status || 'Active', missedDays: l.missed_days_count || 0, collected: Number(l.total_collected || 0), totalCycles: Number(l.total_cycles || 100),
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {loans.length === 0 ? (<tr><td colSpan={8} className="py-8 text-center text-slate-500 font-medium">No borrower profiles registered in the database.</td></tr>) : loans.map((loan) => (
            ...
            <div className="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 relative space-y-4 animate-scaleIn">
              ...
              <div className="space-y-3 pt-2 border-t border-slate-800 text-xs">
                <div className="p-3 rounded-xl bg-slate-950/40 space-y-2">
                  <p className="text-slate-300"><strong>Aadhaar / Gov Code:</strong> {selectedLoanFile.governmentId}</p><p className="text-slate-300"><strong>Home Address:</strong> {selectedLoanFile.address}</p><p className="text-slate-300"><strong>Guarantor Node:</strong> {selectedLoanFile.guarantor}</p><p className="text-slate-300"><strong>Pledged Security Collateral Asset:</strong> {selectedLoanFile.collateral}</p>
                </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-6 relative space-y-4 shadow-2xl animate-scaleIn">
              ...
              <form onSubmit={handleSaveChangesOverride} className="space-y-4 pt-2 border-t border-slate-800 text-xs">
                ...
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold p-3.5 rounded-xl uppercase text-xs mt-2">Commit Operational Profile Updates</button>
              </form>
            </div>
          </div>
        )}
