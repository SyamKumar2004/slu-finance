...
export default function UserSelfRegistrationPortal() {
  ...
  const executeRegistration = async (e: React.FormEvent) => {
    ...
    // 1. Submit Auth User Data to Supabase
    const { data: authData, error: authError } = await supabase.auth.signUp({
      email: form.email,
      password: form.password,
      options: {
        data: {
          email_confirmed: true // Bypasses manual verification email locks
        }
      }
    });

    if (authError) {
      // If the error object is empty or stringified weirdly, show a clear message
      const errorMsg = authError.message || "This email might already be registered. Try signing in instead!";
      alert(`Registration Blocked: ${errorMsg}`);
      setLoading(false);
      return;
    }

    // 2. Create the profile row in user_profiles
    if (authData?.user) {
      ...
      if (profileError) {
        // If profile creation fails because the auth user existed but profile didn't
        alert(`Account exists or profile sync failed: ${profileError.message}`);
      } else {
        alert('Account successfully configured! Redirecting to your dashboard...');
      ...
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
        ...
        <form onSubmit={executeRegistration} className="space-y-4">
          ...
          <div>
            ...
            {/* Added Password Requirement Instruction Text */}
            <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
              <AlertCircle className="h-3 w-3 text-emerald-500 inline" /> Password must be at least 6 characters long.
            </p>
          </div>
