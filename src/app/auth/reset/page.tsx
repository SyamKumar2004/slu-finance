...
export default function CustomProfileResetPortal() {
  ...
  const handleCredentialReset = async (e: React.FormEvent) => {
    ...
    if (!error && data && data.length > 0) {
      ...
      alert("Reset Error: No matching corporate email address trace found inside system indexes.");
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 p-4 font-sans">
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-4">
        <div className="text-center">
          <h2 className="text-2xl font-black text-white flex items-center justify-center gap-2"><ShieldAlert className="text-amber-500" /> Key Override</h2>
        ...
        <form onSubmit={handleCredentialReset} className="space-y-4">
          <div>
            ...
            <div className="relative mt-1">
              <input required type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-3 pl-11 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:outline-none" placeholder="admin@domain.com" />
          ...
          <div>
            ...
            <div className="relative mt-1">
              <input required type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full p-3 pl-11 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:outline-none" placeholder="••••••••••••" />
              <KeyRound className="absolute left-3.5 top-3.5 h-4 w-4 text-slate-500" />
            </div>
          </div>
          <button type="submit" disabled={loading} className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold p-3.5 rounded-xl transition-all uppercase text-xs">
