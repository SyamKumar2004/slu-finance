...
export default function ClientDashboardPortal() {
  ...
  // MEMOIZED LIFE CYCLE FETCH TRACKER: Satisfies strict Vercel loop tracking constraints
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 antialiased p-4 sm:p-6 lg:p-8 font-sans">
      ...
      <header className="flex justify-between items-center mb-8 pb-4 border-b border-slate-900 max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={() => setActiveTab('dossier')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'dossier' ? 'bg-slate-800 text-white border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}>My Credit Dossier</button>
          <button type="button" onClick={() => setActiveTab('security')} className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTab === 'security' ? 'bg-slate-800 text-white border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}>Security Options</button>
          <button type="button" onClick={() => { localStorage.clear(); router.push('/auth/login'); }} className="p-2.5 rounded-xl bg-rose-950/20 text-rose-400 border border-rose-900/20 hover:bg-rose-950/60 transition-all ml-2"><LogOut className="h-4 w-4" /></button>
        </div>
      </header>
        ...
        {activeTab === 'dossier' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {loans.length === 0 ? (
                <div className="md:col-span-3 bg-slate-900 border border-slate-800 rounded-3xl p-8 text-center text-slate-400 font-medium flex flex-col items-center gap-2">
